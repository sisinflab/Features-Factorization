...
public class UserPostCollaborative {
    ...
    private HashMap<Integer, Float> badProfile = new HashMap<>();

    public UserPostCollaborative(HashMap<Integer, ArrayList<Integer>>  map, int user, Map<Integer, Float> votes, HashMap<Integer, Float> badProfile, int numberRecs) {
        this.map = map;
        this.votes = votes;
    ...
    public HashMap<Integer, Float> getProfile(){
        ...
        for (Map.Entry<Integer, ArrayList<Float>> entry : o.entrySet()){
            double featureVal = entry.getValue().get(2)*entry.getValue().get(3);
            if(featureVal > mean){
                profile.put(entry.getKey(), entry.getValue().get(2)*entry.getValue().get(3));
            }
            if(featureVal > mean){
                profile.put(entry.getKey(), 1f);
            }

        }

        badProfile.entrySet().stream().forEach(e -> {
            profile.put(e.getKey(),(-1+e.getValue()));
        });
        return profile;
    }
    ...
    private float score(int item){
        ...
        AtomicReference<Float> rate = new AtomicReference<>(0f);
        HashSet<Integer> bag = new HashSet<Integer>();
        itemFeatures.stream().forEach(feature -> {
            rate.set(rate.get() + userProfile.getOrDefault(feature, 0f));
        });
        return rate.get();
    }

...
public class MFDataModelArray {
    ...
    /** Rating Map           */ private HashMap<Integer, HashMap<Integer, Float>> ratingsMap = new HashMap<>();
    ...
    public MFDataModelArray(HashMap<Integer, HashMap<Integer, Float>> ratingsMap, int numberOfFactors, float initMean, float initStdDev){
    ...
    public HashMap<Integer, HashMap<Integer, Float>> getBadProfiles(){
        HashMap<Integer, HashMap<Integer, Float>> profiles = new HashMap<>();
        HashSet<Integer> candidateItems = new HashSet<>(getItemsSet());
        ratingsMap.keySet().parallelStream().map(user ->{
            HashMap<Integer, Float> profile = new HashMap<>();
            HashSet<Integer> unratedItems = new HashSet<>(candidateItems);
            HashSet<Integer> userItems = new HashSet<>(ratingsMap.get(user).keySet());
            unratedItems.removeAll(userItems);
            unratedItems.stream().forEach(item -> {
                profile.put(item, (float)predictMyMediaLite(user, item));
            });
            HashMap<Integer, Float> profile2 = profile.entrySet().stream().sorted(Entry.comparingByValue()).limit(100)
                    .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue,
                            (v1,v2) -> v1,
                            HashMap::new));

            return new AbstractMap.SimpleEntry<>(user,profile2);
